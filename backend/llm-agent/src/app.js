...
export function createApp({ textGenerator, intentRouter, optionGenerator, browserActionPlanner } = {}) {
  ...
  app.get("/health", (_req, res) => {
    res.json({ ok: true, service: "cursivis-backend", ts: new Date().toISOString() });
  });
      ...
      try {
        ...
        if (isQuotaOrRateLimitError(details)) {
          return res.status(429).json({
            error: "Backend provider quota/rate limit exceeded.",
            details,
  ...
  app.post("/suggest-actions", async (req, res) => {
    ...
    try {
      ...
      return res.status(500).json({
        error: "Suggestion routing failed.",
        details
      });
    }
  });
  ...
  app.post("/transcribe", async (req, res) => {
    ...
    try {
      ...
      if (isQuotaOrRateLimitError(details)) {
        return res.status(429).json({
            error: "Backend provider quota/rate limit exceeded for transcription.",
          details,
          retryAfterSec: extractRetryAfterSeconds(details),
          timestampUtc: new Date().toISOString()
        });
  ...
  app.post("/plan-browser-action", async (req, res) => {
    ...
    try {
      ...
      if (isQuotaOrRateLimitError(details)) {
        return res.status(429).json({
            error: "Backend provider quota/rate limit exceeded for browser planning.",
          details,
          retryAfterSec: extractRetryAfterSeconds(details),
          timestampUtc: new Date().toISOString()
        });
  ...
  app.post("/refine-browser-action", async (req, res) => {
    ...
    try {
      ...
      if (isQuotaOrRateLimitError(details)) {
        return res.status(429).json({
            error: "Backend provider quota/rate limit exceeded for browser plan refinement.",
          details,
          retryAfterSec: extractRetryAfterSeconds(details),
          timestampUtc: new Date().toISOString()
        });
