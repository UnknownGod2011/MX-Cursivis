...
export async function withGoogleGenAiClient(executor, { canRetryError = isRetriableApiKeyError } = {}) {
  ...
  if (pool.entries.length === 0) {
    throw new Error("GOOGLE_API_KEY or GOOGLE_API_KEYS is required to call the configured backend provider.");
  }
  ...
  throw lastError ?? new Error("All configured backend API keys failed.");
}
